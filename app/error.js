...
export default function ErrorPage({ error, unstable_retry }) {
  ...
  return (
    <div className="max-w-lg mx-auto my-24 px-4 text-center">
      <h1 className="text-3xl font-bold text-zinc-800 dark:text-white mb-3">This page had a problem</h1>
      <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-2 break-words">{error.message}</p>
      <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-6">
        {error.digest ? `Error ID: ${error.digest}` : 'Something went wrong while rendering this page.'}
      </p>
      <button
